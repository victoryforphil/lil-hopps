...
impl std::fmt::Display for MotorPhysics {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Force: {:?}, Torque: {:?}, Offset: {:?}",
            self.force, self.torque, self.offset
        )
    }
}

impl Motor {
    pub fn new(num: u8, config: &MotorConfig) -> Motor {
        Motor {
    ...
    pub fn generate_motors(config: &MotorConfig) -> Vec<Motor> {
...
mod tests {
    ...
    fn test_set_input_scalar() {
        let config = MotorConfig::new_250mm();
