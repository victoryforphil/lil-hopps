...
use lil_link::mavlink::{messages::QuadMessageTx, types::QuadTakeoffRequest};
use log::{debug, info};
...
impl System for TimedTakeoff {
    ...
    fn execute<'a>(&mut self, inputs: &'a victory_data_store::database::DataView, dt: victory_wtf::Timespan) -> victory_data_store::database::DataView {
        ...
        info!("Sending takeoff command as time {} has elapsed", self.current_time.secs());
        let takeoff_msg = QuadTakeoffRequest{height: self.height, ack: false};
        self.sent = true;
