...
use lil_link::mavlink::types::{QuadMode, QuadSetModeRequest};
use log::{debug, info};
...
impl System for TimedMode{
    ...
    fn execute<'a>(&mut self, inputs: &'a victory_data_store::database::DataView, dt: victory_wtf::Timespan) -> victory_data_store::database::DataView {
        ...
        info!("Setting mode as time {} has elapsed", self.current_time.secs());
        let mode_msg = QuadSetModeRequest{mode: self.mode.clone(), ack: false};
        self.sent = true;
