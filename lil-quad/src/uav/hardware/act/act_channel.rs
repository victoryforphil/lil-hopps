    ...
    fn test_channel_act(){
        ...
        let inputs = ActHardwareInputs{
            motor_messages: {
                ...
                map.insert("/motor/0".into(), vec![DataPoint::new(t.clone(), 5.0.into())]);
                map
            }
        };
        ...
        assert_eq!(received_inputs.motor_messages.get("/motor/0").unwrap().len(), 1);
        assert_eq!(received_inputs.motor_messages.get("/motor/0").unwrap()[0].data, lil_broker::Primatives::Number(5.0));
    }
}