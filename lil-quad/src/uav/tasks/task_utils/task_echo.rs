...
impl Task for EchoTask{
    fn metadata(&self) -> TaskMetadata{
        TaskMetadata::new("Echo Task".to_string())
            .with_subscriptions(self.echo_topics.iter().map(|topic| TaskSubscription::from(topic.clone())).collect())
...
mod test{
    ...
    fn test_echo_task_run(){
        ...
        let inputs = {
            ...
            map.insert("/topic/0".into(), lil_broker::DataPoint::new(t.clone(), Primatives::Number(5.0)));
            map.insert("/topic/1".into(), lil_broker::DataPoint::new(t.clone(), Primatives::String("lil-hopps".to_string())));
            map
        };
        let result = task.run(&t, &inputs).unwrap();
        assert_eq!(result.data.len(), 2);
        assert_eq!(result.data.get("/topic/0/echo").unwrap().data, Primatives::Number(5.0));
        assert_eq!(result.data.get("/topic/1/echo").unwrap().data, Primatives::String("lil-hopps".to_string()));
    }
}