...
impl Bucket{
    ...
    pub fn range(&self, start: Timestamp, end: Timestamp) -> Vec<DataPoint>{
        let data_points = self.values.range(start..end).map(|(_, dp)| dp.clone()).collect();
        data_points
    }
...
mod tests{
    ...
    fn test_bucket_range(){
        ...
        // Verify latest is none at start
        let latest_value = bucket.range(Timestamp::from_seconds(0.0), Timestamp::from_seconds(10.0));
