...
mod tests {
    ...
    impl SimActor<bool> for TestActor{
        fn init(&mut self, _: SimulationContextHandle, state: &SimulationState) -> Result<bool, String> {
            Ok(true)
        }

        fn step(&mut self, _: SimulationContextHandle, state: &SimulationState, _: f64, _: f64) -> Result<bool, String> {
            Ok(true)
        }
    }
    ...
    fn test_sim_actor() {
        ...
        let state = SimulationState{ uav_state: UAVActorResult::new(UAVState::new(Pose::zero())), world_state: world_actor::WorldActorResult {  } , running : false };
