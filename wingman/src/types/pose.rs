...
impl std::fmt::Display for Pose{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result{
        ...
        write!(f, "Position: [{:.4},{:.4},{:.4}], Orientation: {:?}", self.position.x, self.position.y , self.position.z, self.orientation)
    }
}

impl Pose{
    pub fn new(position: nalgebra::Vector3<f64>, orientation: nalgebra::UnitQuaternion<f64>) -> Pose{
        Pose{
            position,
            orientation
        }
...
mod tests{
    ...
    fn test_pose_display(){
        ...
        assert_eq!(result, "Position: [1.0000,2.0000,3.0000], Orientation: [0.0, 0.0, 0.0, 1.0]");
    }
}
